...
class TextEnhancer:
    ...
    def _init_client(self):
        ...
        if self.groq_api_key:
            try:
                ...
                self.backend = "groq"
                self.model = "llama3-8b-8192"   # fast, free Groq model
                log_info("Enhancer using Groq (free)")
                return
        ...
        if self.openai_api_key:
            try:
                ...
                log_info("Enhancer using OpenAI GPT")
                return
            except Exception as e:
    ...
    def enhance(self, text: str, mode: str = "formal") -> Optional[str]:
        ...
        try:
            ...
            log_debug(f"Language detected: {lang_name} ({lang_code}), script: {script}, confidence: {confidence:.2f}")
