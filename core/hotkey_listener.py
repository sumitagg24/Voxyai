...
UNSAFE_COMBOS = {
    "win", "win+d", "win+e", "win+l", "win+r", "win+s",
    "ctrl+c", "ctrl+v", "ctrl+x", "ctrl+z", "ctrl+shift+v",
    "alt+f4", "alt+tab",
}
...
class _SingleHotkey:
    ...
    def __init__(self, hotkey: str, on_press: Callable, on_release: Callable,
                 toggle_mode: bool = False, mode_override: Optional[str] = None):
        self.hotkey = hotkey
    ...
    def handle_press(self, held: Set):
        canon_held = held
        if all(_canonical(k) in canon_held or k in canon_held for k in self._target_keys):
            if not self._active:
                self._active = True
                if self.toggle_mode:
...
class HotkeyListener:
    ...
    def _build_hotkeys(self):
        ...
        self._hotkeys.append(_SingleHotkey(
            self.hotkey,
    ...
    def set_mode_hotkeys(self, mode_hotkeys: Dict[str, str]):
        ...
        if was: self.stop_listening()
        self._build_hotkeys()
        if was: self.start_listening()
    ...
    def _fire_pressed(self):
        ...
        if self.on_hotkey_pressed:
            try: self.on_hotkey_pressed()
            except Exception as e: log_error(f"on_hotkey_pressed: {e}", exc_info=True)

    def _fire_released(self):
        event_manager.emit(Events.HOTKEY_RELEASED)
        if self.on_hotkey_released:
            try: self.on_hotkey_released()
            except Exception as e: log_error(f"on_hotkey_released: {e}", exc_info=True)

    def _fire_mode_pressed(self, mode: str):
        event_manager.emit(Events.MODE_HOTKEY_PRESSED, mode)
        if self.on_mode_hotkey:
            try: self.on_mode_hotkey(mode, "press")
            except Exception as e: log_error(f"on_mode_hotkey: {e}", exc_info=True)

    def _fire_mode_released(self, mode: str):
        event_manager.emit(Events.MODE_HOTKEY_RELEASED, mode)
        if self.on_mode_hotkey:
            try: self.on_mode_hotkey(mode, "release")
            except Exception as e: log_error(f"on_mode_hotkey: {e}", exc_info=True)
