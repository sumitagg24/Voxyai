...
class CommandProcessor:
    ...
    def __init__(self):
        self.snippets_file = Path("config/snippets.json")
    ...
    def process_command(self, text: str) -> Tuple[bool, str]:
        ...
        if self._is_web_search(text):
            ...
            return False, "Command not recognized. Try: search, create issue, draft email, clean text, save snippet, or send Slack message"
    ...
    def _extract_search_query(self, text: str) -> Optional[str]:
        ...
        query = re.sub(r"^(search|look|find|google|search the web)\s+(for|about|up)?\s*", "", text, flags=re.IGNORECASE)
        query = query.strip()
    ...
    def _handle_github(self, text: str) -> Tuple[bool, str]:
        ...
        if not GITHUB_AVAILABLE:
            return False, "GitHub integration not available. Install: pip install PyGithub"
    ...
    def _extract_github_title(self, text: str) -> Optional[str]:
        ...
        # Remove common prefixes
        title = re.sub(r"^(create|report)\s+(a\s+)?(github\s+)?(issue|bug)\s+(titled|called)?\s*", "", text, flags=re.IGNORECASE)
        title = title.strip()
    ...
    def _handle_email(self, text: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return True, f"Email draft created and copied to clipboard:\n\nTo: {recipient}\nSubject: {subject}"
        except Exception as e:
    ...
    def _extract_email_info(self, text: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        match = re.search(r"to\s+(\w+)\s+(?:about|regarding|with subject)\s+(.+?)(?:\.|$)", text, re.IGNORECASE)
        if match:
...
"""
    ...
    def _extract_text_to_clean(self, text: str) -> Optional[str]:
        ...
        match = re.search(r"(?:clean|make|polish|improve|fix)\s+(?:this\s+)?(?:sentence|text|message)?\s*[:\"]?\s*(.+?)(?:\.|$)", text, re.IGNORECASE)
        if match:
    ...
    def _handle_slack(self, text: str) -> Tuple[bool, str]:
        ...
        if not SLACK_AVAILABLE:
            return False, "Slack integration not available. Install: pip install slack-sdk"
    ...
    def _extract_slack_message(self, text: str) -> Optional[str]:
        ...
        match = re.search(r"(?:send\s+)?slack\s+(?:message)?\s*[:\"]?\s*(.+?)(?:\.|$)", text, re.IGNORECASE)
        if match:
            return match.group(1).strip()
        return None
